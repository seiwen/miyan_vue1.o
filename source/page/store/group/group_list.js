require([
	...
	'lib/moment.js',
], function($, Vue, common, lazyload, moment) {
    var type = common.getUrlData('type');
	if (type == 0) {
		$(document).attr('title', '全部拼团');
	}else if (type == 1) {
		$(document).attr('title', '待成团');
	}else if (type == 2) {
		$(document).attr('title', '拼团成功');
	}else if (type == 3) {
		$(document).attr('title', '拼团失败');
	}
	//时间过滤器
	...
	function getMoreRecru(flag) {
    	common.globalAjax({
			action: 'Order.GetPinTuanOrderList',
			data: {
				type: type,
				p: flag
			},
