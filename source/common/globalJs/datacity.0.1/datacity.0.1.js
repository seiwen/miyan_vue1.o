...
function preselect(keyCity) {
    ...
    var cnt = provinces.length;  //数组34的个数

    for (i = 0; i < cnt; i++) {
        if (opt.key == provinces[i]) {
            index = i;
            break;
        }
    }
	...
	//判断城市
	if(opt.city){
	   var iArray=dsy.Items;
	   var key="0_"+index;
	   var cityisArtay=iArray[key];
	   var le=cityisArtay.length;
	   for(i=0;i<le;i++){
		   if(opt.city==cityisArtay[i]){
				init=i
				break;
			   }
		   }
	 if(index < provinces.length) {
			document.getElementById(s[1]).selectedIndex = init + 1;
			change(2);
		}
	}
	...
	if(opt.country){
       var iArray=dsy.Items;
	   var key="0_"+index+"_"+init;
	   var cityisArtay=iArray[key];
	   var le=cityisArtay.length;
		for(i=0;i<le;i++){
			if(opt.country==cityisArtay[i]){
				 st=i
				 break;
				}
			}
		if(init<provinces.length){
			document.getElementById(s[2]).selectedIndex = st + 1;
			}	
		}
}
