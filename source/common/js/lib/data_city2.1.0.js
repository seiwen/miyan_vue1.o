...
dsy.add("0_0_0", ["东城区", "西城区", "崇文区", "宣武区", "朝阳区", "丰台区", "石景山区", "海淀区", "门头沟区", "房山区", "通州区", "顺义区", "昌平区", "大兴区", "怀柔区", "平谷区", "密云区", "延庆县"]);
dsy.add("0_0", ["北京市"]);
...
dsy.add("0_3_0", ["渝中区", "大渡口区", "江北区", "沙坪坝区", "九龙坡区", "南岸区", "北碚区", "万盛区", "高新区", "北部新区","双桥区", "渝北区", "巴南区", "万州区", "涪陵区", "黔江区", "长寿区", "合川区", "永川区", "江津区", "南川区", "綦江区", "潼南县", "铜梁县", "大足区", "荣昌县", "璧山县", "垫江县", "武隆县", "丰都县", "城口县", "梁平县", "开县", "巫溪县", "巫山县", "奉节县", "云阳县", "忠县", "石柱土家族自治县", "彭水苗族土家族自治县", "酉阳土家族苗族自治县", "秀山土家族苗族自治县"]);
dsy.add("0_3", ["重庆市"]);
...
dsy.add("0_10_11", ["梁溪区", "滨湖区", "惠山区", "锡山区", "江阴市", "宜兴市", "新吴区"]]);
...
dsy.add("0_19_0", ["越秀区", "荔湾区", "海珠区", "天河区", "白云区", "黄埔区", "番禺区", "花都区", "南沙区", "萝岗区", "增城区", "从化区", "广州大学城"]);
...
dsy.add("0_19_11", ["莞城区","东城区","南城区","万江区","石龙镇","石排镇","茶山镇","企石镇","桥头镇","东坑镇","横沥镇","常平镇","虎门镇","长安镇","沙田镇","厚街镇","寮步镇","大岭山镇","大朗镇","黄江镇","樟木头镇","谢岗镇","塘厦镇","清溪镇","凤岗镇","麻涌镇","中堂镇","望牛墩镇","洪梅镇","道滘镇","高埗镇","石碣镇",]);
...
function setup() {
   var contentprovince = '',contentCity = '',contentCountry = '',province='',city ='';
	for(var all in dsy.Items){
		var alls = dsy.Items[all];
		if(all == "0"){
			for(var i = 0; i < alls.length; i++){
				contentprovince += '<li data-province="0_' + i + '">' + alls[i] + '</li>';
			};
		};
	}
	$('#province').html(contentprovince);

	$("#province li").click(function(event) {
		var index = $(this).attr("data-province");
		$("#province").hide();
		$("#province li").remove();
		$('#city').show();
		for(var all in dsy.Items){
			...
			if(all == index){
				for(var i = 0; i < alls.length; i++){
					contentCity += '<li data-city="' + index + '_' + i + '">' + alls[i] + '</li>';
				};
			};
		}
		$('#city').html(contentCity);
		province = $(this).html();
		$("#provinceTxt-nav").html(province);
		$("#cityTxt-nav").show();
		$("#cityTxt-nav").addClass('active').siblings().removeClass('active');
		...
		$("#city li").click(function(event) {
			...
			for(var all in dsy.Items){
				...
				if(all == index){
					for(var i = 0; i < alls.length; i++){
						contentCountry += '<li data-country="' + index + '_' + i + '">' + alls[i] + '</li>';
					};
				};
			}
			$('#country').html(contentCountry);
			city = $(this).html();
			$("#cityTxt-nav").html(city);
			$("#countryTxt-nav").show();
			$("#countryTxt-nav").addClass('active').siblings().removeClass('active');

			$('#country li').click(function(event) {
				$("#countryTxt-nav").html($(this).html());
				$("#provinceTxt-nav").addClass('active').siblings().removeClass('active');
				...
				$("#provinceTxt").html(province)
				$("#cityTxt").html(city );
				$("#countryTxt").html($(this).html());
				$("#provinceTxt-nav,#cityTxt-nav,#countryTxt-nav").html('请选择');
