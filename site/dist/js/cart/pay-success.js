	            ...
	            common.globalAjax({
	                action: 'Incentive.incentiveIndex',
	                    ...
	                    if(week_tiduNum_length>3){
	                        ...
	                        paySuccess.$set('week_task', week_task_arr);
	                        var week_length = 3;
	                    }
	                ...
	                if(month_tiduNum_length>3){
	                    ...
	                    paySuccess.$set('month_task', month_task_arr);
	                    var month_length = 3;
	                }
	                ...
	                var task_data = res.data.info.one_order_task;
	                var man_length = task_data.length;
	                var max_data = [];
	                var money = paySuccess.info.incentive_price;//排除限制商品的价格
	                //循环输出少于订单价格的规则
	                for (i in task_data) {
	                    if(task_data[i].man < (money+1)){
	                        max_data.push(task_data[i].man);
	                    }
	                }
	                var man_task = Math.max.apply(null, max_data);
	                var max_task = [];
	                for (val in task_data) {
	                    if(task_data[val].man == man_task){
	                        max_task = task_data[val]
	                    }
	                }
	                setTimeout(function(){
	                    if(max_task.man == undefined){
	                        $('.one-order-task').hide();
	                    }
	                    $('.max-task').append('单笔订单满'+max_task.man+'元，<label>奖励'+max_task.jiang+'元现金</label>')
	                }, 100);
	                paySuccess.$set('task_info', res.data.info);
	            }
	        })
	    }
	    }
	    var paySuccess = new Vue({
	        ...
	        ready: function() {
	            ...
	            common.globalAjax({
	            	...
	            	data: {
	            		...
	            		paySuccess.$set('info', res.data.info);
	                    if(is_open != 'MZSHOP'){
	                        task();
	                    }else{
	                        window.location.href = 'callApp://weShopPaySuccess';
	                        cookie.set('isWeShopPayment',1,{ expires: 1 });//开店付费成功后写入cookie，表明付费成功
	                    }
	                    $('.root').show();
	                    setTimeout(function() {
	                        //传APP参数控制整个页面的返回键
	                        if(common.checkAPP() == 'miyan'){
	                            window.location.href = 'callApp://walletPayBack';
	                        }
	                    }, 500);
	            	},
	            	fail: function(res) {
	        ...
	        methods: {
	            gotoPage: function(page) {
	                var back_open_checkout = 'http://'+window.location.host+'/page/user/open-checkout.html?yq_code='+cookie.get('yq_code');
	                switch(page) {
	                    case 'index':
	                    ...
	                    case 'fail':
	                        ...
	                        break;
	                    case 'go':
	                        window.location.href = back_open_checkout;
	                        break;
	                    case 'invite_shop':
	                        //进入邀请开店页
	                        window.location.href = '/page/user/invite-shop.html?is_invite=1';
	                        break;
	                }
	            },
	            gotoReward: function() {
	                window.location.href = '/page/user/reward.html';
	            },
	            goDownload: function(){
