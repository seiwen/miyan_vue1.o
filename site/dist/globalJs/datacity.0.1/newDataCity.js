...
dsy.add("0_19_1", ["福田区", "罗湖区", "南山区", "宝安区", "龙岗区", "盐田区", "光明新区", "坪山新区", "大鹏新区", "龙华新区"]);
...
dsy.add("0_34_0", ["钓鱼岛"]);
dsy.add("0_34", ["钓鱼岛"]);
dsy.add("0", ["北京市", "天津市", "上海市", "重庆市", "河北省", "山西省", "内蒙古", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西省", "海南省", "四川省", "贵州省", "云南省", "西藏", "陕西省", "甘肃省", "青海省", "宁夏", "新疆", "香港", "澳门", "台湾省", "钓鱼岛"]);

function setup() {
   var contentKey = '',contentCity = '',contentCountry = '',key='',city ='',ss='';
	for(var all in dsy.Items){
		var alls = dsy.Items[all];
		if(all == "0"){
			for(var i = 0; i < alls.length; i++){
				contentKey += '<li data-key="0_' + i + '">' + alls[i] + '</li>';
			};
		};
	}
	$('#key').html(contentKey);

	$("#key li").click(function(event) {
		...
		$("#city li").click(function(event) {
			...
			$('#country li').click(function(event) {
				$(".addressBox").removeClass('bounceInRight')
		        $(".addressBox").addClass('bounceOutRight')
		        setTimeout(function(){
		            $("#country,.addressBox,.hideBox").hide();
		            $('#country li,#city li').remove();
		            $("#key").show();
		            $(".addressBox").removeClass('bounceOutRight');
		            $(".addressBox").addClass('bounceInRight');
		            $("html,body").css({
		                overflow: 'auto',
		                height: 'auto'
		            });
